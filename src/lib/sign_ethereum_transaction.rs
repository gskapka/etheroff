...
fn sign_transaction(state: State) -> Result<String> {
    ...
    info!(
        "✔ For amount: {} Wei",
        state.value.as_u64().to_formatted_string(&Locale::en)
    );
    info!(
        "✔ With gas limit of: {}",
        state.gas_limit.as_u64().to_formatted_string(&Locale::en)
    );
    info!(
        "✔ And a gas price of: {} Wei",
        state.gas_price.as_u64().to_formatted_string(&Locale::en)
    );
